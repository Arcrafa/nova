...
def kProngDispl(tables):
    dfvtx = tables['rec.vtx.elastic'][['vtx.x', 'vtx.y', 'vtx.z']]
    dfpng = tables['rec.vtx.elastic.fuzzyk.png'][['start.x', 'start.y', 'start.z', 'rec.vtx.elastic.fuzzyk.png_idx']]
    ...
    length.name = 'disp'
    idx = dfpng['rec.vtx.elastic.fuzzyk.png_idx']
...
def kIgnoreMuon2018(tables):
    dfpng = tables['rec.vtx.elastic.fuzzyk.png.cvnpart']['muonid']
    dflen = tables['rec.vtx.elastic.fuzzyk.png']['len']
    if not dfpng.empty:
      dflen = dflen.reset_index().set_index(KL)
      dfpng = dfpng.reset_index().set_index(KL)
      maxid = dfpng['muonid'].groupby(level=KL).agg(np.max)
      ret = pd.concat([dfpng['muonid'] - maxid, dfpng['rec.vtx.elastic.fuzzyk.png_idx'], dflen['len']], axis=1)
      ret = ret.reset_index().set_index(KLP)
