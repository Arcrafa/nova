...
class samprojectsource():
  ...
  def establishProcess(self):
    ...
    self.processID = self.ifdh.establishProcess(self.projurl, "PandAna", "0.1",
                                                os.getenv('HOSTNAME'),
...
class samquerysource(samprojectsource):
  ...
  def __init__(self, query):
    import samweb_client
    self.SAMclient = samweb_client # need the whole module for exceptions
    self.SAM = samweb_client.SAMWebClient(experiment='nova')
  ...
  def setupProject(self):
    ...
    if self.nProjWithPrefix(projname+checkprojid) > MAX_PROJECTS:
      print (
      """
    ...
    if not self.isgrid:
      ...
      try:
        # lock creation of projects for other processes
        self.SAM.createDefinition(projlock, "nova.special %s" % os.getenv('PROCESS'))
        # No one's created our project. We can go for it
        self.SAM.startProject(projname+"_"+uniqueid, defname=self.definition, snapshot_id=self.snapshot_id)
        projname += "_"+uniqueid

      except self.SAMclient.exceptions.SAMWebConflict:
        # waiting for another process to create it 
        period = 1
        while not self.nProjWithPrefix(projname):
          time.sleep(int(period))
          period *= 1.5
          if period > 60*30:
            print ("""
            We've been waiting a long time for the project to be created. 
            We don't think its happening.
            """)
            sys.exit(2)
        assert self.nProjWithPrefix(projname) == 1, \
        """
        Multiple projects of the same loader instance running on the same cluster.
        This shouldn't be possible
        """
        projlist = self.SAM.listProjects(defname=self.definition, snapshot_id=self.snapshot_id, state='running')
        # Some other grid job has already created our project. Connect to it
        projname = projlist[[projname in l for l in projlist].index(True)]
  ...
  def isproj(self):
    ...
    with open(os.devnull, 'wb') as null:
      try:
        ...
        exitcode = subprocess.check_call(['bash', '-c', 'ifdh projectStatus {}'.format(url)], 
  ...
  def __call__(self):
    if self.islist():
      ...
      if self.stride > 1 or self.offset > 0:
        print ("""
        Can't use stride and offset for SAM projects. 
        Use a query instead to start an internal project
        """)
        sys.exit(2)
      if os.getenv('PANDANA_LIMIT'):
        self.limit = int(os.getenv('PANDANA_LIMIT'))
