...
class Cut():
    ...
    def __invert__(self):
        cut = Cut(self._cut[0], not self._invert[0])
        for i in range(1, len(self._cut)):
          cut = cut | Cut(self._cut[i], not self._invert[i])
        return cut
...
class loader():
    def __init__(self, filesource, stride = 1, offset = 0, limit = None, spillcuts=None, index=None):
        ...
        self.index=index
        self.dflist = {}
        self._spillcuts = spillcuts

        # add extra spectra to keep track of exposure
        self._POT = 0
        self.sum_POT()
    ...
    def sum_POT(self):
        ...
        # If not gone, construct spectra
        if not self.gone:
            self._potspecnocut = spectrum(self, lambda tables: tables['spill']['spillpot']>0, \
                                              lambda tables: tables['spill']['spillpot'])

            if self._spillcuts:
                self._potspeccut = spectrum(self, self._spillcuts, \
                                                lambda tables: tables['spill']['spillpot'])
            else:
                self._potspeccut = None

        # If gone, fill spectra and compute POT
        else:
            self._potspecnocut.fill()
            self._POTBase = self._potspecnocut.df().sum()
            if self._potspeccut:
                self._potspeccut.fill()
                self._POT = self._potspeccut.df().sum()
            else:
                # Use base pot if not using spill cuts
                self._POT = self._POTBase
            frac = 100*self._POT/self._POTBase
            print('Found {:0.5E} POT passing spillcuts from {:0.5E} POT ({:0.1f}%).'.format(self._POT, self._POTBase, frac))
        self.summing = False

    def add_spectrum(self, spec):
        if not spec in self.histdefs:
            if self._spillcuts is not None and not self.summing:
                spec._cutfcn = spec._cutfcn & self._spillcuts
    ...
    def __getitem__(self, key):
        if not self.summing:
            if type(key)==str and key.startswith('spill'):
                key = 'rec.'+key
        ...
        # use global index to slice dataframe requested
            ...
            if self._tables[key].index.intersection(self._tables['indices']).empty:
              return dfproxy(columns=self._tables[key].index.names)
            else:
    ...
    def readData(self):
        for key in self._tables:
            ...
            if not key in self.dflist:
                self.dflist[key] = []
            # branches from cache
            if not key in self.openfile.keys():
              print("Group %s doesn't exist!" % key)
              sys.exit(2)
            ...
            for k in keycache:
                try:
                  ...
                  print("Dataset %s for group %s doesn't exist!" % (k, group))
                  sys.exit(2)
    ...
    def fillSpectra(self):
        self.concat_time = time.time()
        for key in self.dflist:
            # set index for all dataframes
            self[key] = pd.concat(self.dflist[key])
            # sort index
            self._tables[key].sort_index(inplace=True)
        self.dflist = {}

        # Compute POT and then fill spectra
        self.sum_POT()

        # Let's not refill these
        self.histdefs.remove(self._potspecnocut)
        if self._potspeccut:
            self.histdefs.remove(self._potspeccut)
